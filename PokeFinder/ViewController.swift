...
//  PokeFinder
//
//  Created by Jelmar Van Aert on 19/02/2017.
...
class ViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    func centerMapOnLocation(location: CLLocation) {
        let coordinatRegion = MKCoordinateRegionMakeWithDistance(location.coordinate, 2000, 2000)
    ...
    func showSightingsOnMap(location: CLLocation) {
        ...
        _ = circleQuery?.observe(GFEventType.keyEntered) { (key, location) in
            ...
            if let key = key, let location = location {
                let anno = PokeAnnotaion(coordinate: location.coordinate, pokemonNumber: Int(key)!)
